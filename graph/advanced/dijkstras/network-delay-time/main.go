...
func networkDelayTime(times [][]int, n, k int) int {
	...
	// If the source node has no neighbours, then the signal cannot propogate in the network
	if len(adjacencyList[k]) == 0 {
		return -1
	}

	// Initialize the min heap and push the origin `src` node
	minHeap := &MinHeap{}
	heap.Push(minHeap, Node{id: k, cost: 0})
	...
	// To capture the minimum time for the signal to reach all nodes, we take the maximum value in the shortest path map.
	// If the farthest node receives the signal, then all other nearer nodes from the src would have already received it.
	var minTime int
	for _, val := range shortest {
		minTime = max(minTime, val)
	}

	// Once the shortest path map is formed, if the length of the path is not the same as the number
	// of the nodes in the graph => From a given `src` node, we cannot reach all parts of the nodes
	// Hence, we return -1
	if len(shortest) != n {
		return -1
	}
